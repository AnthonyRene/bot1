...
module.exports = {
    name: "play",
    description: "Spielen Sie Ihre Lieblingslieder",
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        let Searching = await message.channel.send(":mag_right: Suche Läuft..");
       ...
       return client.sendTime(message.channel,"❌ | Lavalink nicht connected.");
        ...
        let SongAddedEmbed = new MessageEmbed().setColor("0x00AE86");
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.setAuthor(`Wiedergabeliste zur Warteschlange hinzugefügt`, message.author.displayAvatarURL());
                    SongAddedEmbed.addField("In die Warteschlange werden", `\`${Searched.tracks.length}\` songs Hinzugefügt`, false);
                    ...
                    if (player.queue.totalSize > 1) SongAddedEmbed.addField("Position in der Warteschlange", `${player.queue.size - 0}`, true);
                    Searching.edit(SongAddedEmbed);
                } else {
                    return client.sendTime(message.channel, "**Keine Übereinstimmungen gefunden für - **" + SearchString);
                }
            } else {
                ...
                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**Keine Übereinstimmungen gefunden für - **" + SearchString);
                    ...
                    if (!player.playing && !player.paused && player.queue.totalSize === Searched.tracks.length) player.play();
                    SongAddedEmbed.setAuthor(`Wiedergabeliste zur Warteschlange hinzugefügt`, client.config.IconURL);
                    SongAddedEmbed.setThumbnail(Searched.tracks[0].displayThumbnail());
                    SongAddedEmbed.setDescription(`[${Searched.playlist.name}](${SearchString})`);
                    SongAddedEmbed.addField("In die Warteschlange gestellt", `\`${Searched.tracks.length}\` songs`, false);
                    SongAddedEmbed.addField("Dauer der Wiedergabeliste", `\`${prettyMilliseconds(Searched.playlist.duration, { colonNotation: true })}\``, false);
                    Searching.edit(SongAddedEmbed);
                } else {
                    ...
                    SongAddedEmbed.addField("Dauer", `\`${prettyMilliseconds(Searched.tracks[0].duration, { colonNotation: true })}\``, true);
                    if (player.queue.totalSize > 1) SongAddedEmbed.addField("Position in der Warteschlange", `${player.queue.size - 0}`, true);
                    Searching.edit(SongAddedEmbed);
                }
            }
    ...
    SlashCommand: {
        options: [
            {
                ...
                type: 3,
                required: true,
                description: "Spielen Sie Musik im Sprachkanal ab",
            },
        ],
        /**
         *
        ...
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (guild.me.voice.channel && !guild.me.voice.channel.equals(member.voice.channel)) return client.sendTime(interaction, `❌ | **Sie müssen sich im selben Sprachkanal befinden wie der Bot, um etwas zu spielen!**`);
            let CheckNode = client.Manager.nodes.get(client.config.Lavalink.id);
            if (!CheckNode || !CheckNode.connected) {
              return client.sendTime(interaction,"❌ | Lavalink nicht connected.");
            }
            ...
            if (search.match(client.Lavasfy.spotifyPattern)) {
                ...
                switch (Searched.loadType) {
                    ...
                    case "TRACK_LOADED":
                        ...
                        return interaction.send(`**Gesuchter Track** \`${Searched.tracks[0].info.title}\`.`);
                ...
                try {
                    ...
                    return interaction.send(`Beim Suchen ist ein Fehler aufgetreten: ${err.message}`);
                }
                switch (res.loadType) {
                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
                    ...
                    case "TRACK_LOADED":
                        ...
                        if (!player.playing && !player.paused && !player.queue.length) player.play();
                        return client.sendTime(interaction, `**Zur Warteschlange hinzugefügt** \n[${res.tracks[0].title}](${res.tracks[0].uri})`);
                    case "PLAYLIST_LOADED":
                        player.queue.add(res.tracks);

                        if (!player.playing && !player.paused && player.queue.size === res.tracks.length) player.play();
                        return client.sendTime(interaction, `**Gesuchte Wiedergabeliste**: \n **${res.playlist.name}** : **${res.tracks.length} tracks**`);
                    case "SEARCH_RESULT":
                        const track = res.tracks[0];
                        player.queue.add(track);

                        if (!player.playing && !player.paused && !player.queue.length) {
                            client.sendTime(interaction, `**Zur Warteschlange hinzugefügt** [${res.tracks[0].title}](${res.tracks[0].uri})`);
                            player.play();
                        } else {
